...
API_TOKEN = '7925979596:AAESYfDg68tbE3gouUwZSzy2JtB0zfrF6jo'  # Вставь свой токен
ADMIN_ID = 3172037  # Твой ID
...
async def service_chosen(call: CallbackQuery, state: FSMContext):
    ...
    # Даты начиная с послезавтра (2-й день), 5 дней
    for i in range(2, 7):
...
async def time_chosen(call: CallbackQuery, state: FSMContext):
    ...
    # Защита: не более 2 записей в день
    ...
    # Защита: не более 1 на одно и то же время
    ...
    reply_text = (
        ...
        f"💳 2202 2006 1234 5678\n"
        f"Получатель: Иван Иванов\n\n"
        f"❗ Для подтверждения записи оплатите пожалуйста по реквизитам"
    )
